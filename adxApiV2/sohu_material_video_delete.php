...
// 调入配置参数
$app_name = "sohu_material_video_delete";
...
$account_db_sql = "select accountId,uploadName,customerKey from center_account where exchangeStatus=1 and exchangeId=17";
...
//查询需要上传的素材Id
    ...
    if ($uploadMaterialList) {
        foreach ($uploadMaterialList as $material_item) {
            ...
            $file_url = $config->video_file_path . $material_item["fileUrl"];
            $sizeId = $material_item['width'] . '×' . $material_item['height'];
            $size = round(filesize($config->video_file_system_path . $material_item["fileUrl"]) / 1024, 2);
