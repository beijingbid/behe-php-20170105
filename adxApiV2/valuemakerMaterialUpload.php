...
class valuemakerMaterialUpload extends base{

    public $appName = 'valuemakerMaterialUpload';
    ...
    public function run(){
        ...
        $arr_db_info = $this->getBannerMaterial(27,'3,99','0,1');
        if ($arr_db_info) {
            ...
            foreach ($uploadMaterialList as $item) {
                ...
                if($item['uploadStatus'] == 0):
                    ...
                    if ($str_return['httpCode'] == 200) {
                        ...
                        $this->saveMaterial("adxId = 27 and advertId = {$materialId}",$update_data);
                    } else {
        ...
        $allowAccountIdAry = $this->getAllowAdvertiser(27);
        $uploadMaterialList = array();
        $orderIdAry = array();
        $campaignIdAry = array();
        $adAccountIdAry = array();
        $arr_db_info = $this->getVideoMaterial(27,'3,99','0,1');
        if ($arr_db_info) {
            foreach ($arr_db_info as $item_db) {
                $orderId = $item_db["orderId"];
                $orderIdAry[$orderId] = $orderId;
...
$obj = new valuemakerMaterialUpload();
$obj->run();
?>
