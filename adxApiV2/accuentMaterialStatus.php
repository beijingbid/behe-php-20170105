...
class accuentMaterialStatus extends base{

    public $appName = 'accuentMaterialStatus';
    ...
    public function run(){
        ...
        $arr_material_list = $this->getWaitStatusMaterial(53);
        ...
        if ($arr_material_list) {
            foreach ($arr_material_list as $k => $item) {
                ...
                if (!empty($str_return)) {
                    ...
                    if (!empty($updateData)) {
                        $this->saveMaterial("adxId = 53 and advertId = " . $materialId,$updateData);
